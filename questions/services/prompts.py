...
- ★★★重要★★★ KaTeX数式（$...$や$$...$$）をonclick属性を持つ要素の内部に配置しないでください。
  数式クリックでAI導出説明のポップアップが開くため、onclick付き要素（展開/折りたたみ、タブ切替等）の
  中に数式があると、クリック時に意図しない折りたたみ/展開が発生します。
  数式は展開/折りたたみの対象コンテンツ内に入れてください（トリガー要素自体には入れない）。
